...
// @updateURL    https://raw.githubusercontent.com/matthewm1805/tampermonkey/main/Auto%20ChatGPT-1.0.user.js
// @downloadURL  https://raw.githubusercontent.com/matthewm1805/tampermonkey/main/Auto%20ChatGPT-1.0.user.js
// ==/UserScript==
...
(function() {
    ...
    const DIALOGUES_STORAGE_KEY = 'savedDialogues_v2_6_reorder';
    const MINIMIZED_STATE_KEY = 'autoChatGPTMinimizedState_v2_6';
    const WAIT_MIN_SECONDS_KEY = 'autoChatGPTWaitMinSec_v2_6';
    const WAIT_MAX_SECONDS_KEY = 'autoChatGPTWaitMaxSec_v2_6';
    ...
    function updatePromptNumbers() {
        ...
        if (!container) return;
        const wrappers = container.querySelectorAll(`:scope > .${ITEM_WRAPPER_CLASS}`);
        wrappers.forEach((wrapper, index) => {
            const numberSpan = wrapper.querySelector(`.${PROMPT_NUMBER_CLASS}`);
            if (numberSpan) {
    ...
    function createUI() {
        ...
        document.addEventListener('mouseleave', handlePanelDragEnd);
    ...
    function toggleMinimize() {
        ...
        const dot = document.getElementById(MINIMIZE_DOT_ID);
        if (!panel || !dot) return;
    ...
    function updateDialogueDataArray() {
        ...
        dialogueData = Array.from(container.querySelectorAll(`:scope > .${ITEM_WRAPPER_CLASS}`))
            .map(wrapper => {
                const textarea = wrapper.querySelector('.dialogue-textarea');
                const repeatInput = wrapper.querySelector('.repeat-input');
                const promptId = wrapper.getAttribute('data-id');
    ...
    function updateEmptyStateVisibility() {
        ...
        const hasPrompts = container.querySelector(`:scope > .${ITEM_WRAPPER_CLASS}`) !== null;
        emptyState.style.display = hasPrompts ? 'none' : 'flex';
        addFooterBtn.style.display = hasPrompts ? 'flex' : 'none';
    }
    ...
    function addDialogueInput(data = { text: '', repeat: 1 }, insertAfterElement = null) {
        ...
        inputWrapper.className = ITEM_WRAPPER_CLASS;
        inputWrapper.setAttribute('data-id', promptId);

        const promptNumberSpan = document.createElement('span');
        promptNumberSpan.className = PROMPT_NUMBER_CLASS;
        inputWrapper.appendChild(promptNumberSpan);

        const activeDot = document.createElement('span');
        activeDot.className = ACTIVE_PROMPT_DOT_CLASS;
        inputWrapper.appendChild(activeDot);

        const contentDiv = document.createElement('div');
        contentDiv.className = 'dialogue-content';

        const textarea = document.createElement('textarea');
        textarea.className = 'dialogue-textarea';
        textarea.placeholder = 'Nhập hoặc dán câu thoại...';
        textarea.rows = 1;
        ...
        repeatLabel.textContent = 'Lặp:';
        repeatLabel.htmlFor = `repeat-${promptId}`;

        const repeatInput = document.createElement('input');
        repeatInput.className = 'repeat-input';
        repeatInput.type = 'number';
        repeatInput.id = `repeat-${promptId}`;
        repeatInput.min = '1';
        repeatInput.value = data.repeat > 0 ? data.repeat : 1;
        repeatInput.title = 'Số lần lặp lại câu thoại này';
        repeatInput.addEventListener('change', saveDialogues);
        repeatInput.addEventListener('input', saveDialogues);

        const moveUpBtn = document.createElement('button');
        moveUpBtn.className = 'move-up-btn icon-button';
        moveUpBtn.title = 'Di chuyển lên';
        moveUpBtn.innerHTML = UP_ARROW_SVG;
        moveUpBtn.addEventListener('click', handleMoveUp);

        const moveDownBtn = document.createElement('button');
        moveDownBtn.className = 'move-down-btn icon-button';
        moveDownBtn.title = 'Di chuyển xuống';
        moveDownBtn.innerHTML = DOWN_ARROW_SVG;
        moveDownBtn.addEventListener('click', handleMoveDown);

        const addBtn = document.createElement('button');
        addBtn.className = 'add-prompt-btn icon-button';
        addBtn.title = 'Thêm câu thoại mới bên dưới';
        addBtn.innerHTML = PLUS_SVG;
        addBtn.addEventListener('click', (e) => {
            const currentWrapper = e.currentTarget.closest(`.${ITEM_WRAPPER_CLASS}`);
            addDialogueInput(undefined, currentWrapper);
        });

        const removeBtn = document.createElement('button');
        removeBtn.className = 'remove-prompt-btn icon-button';
        removeBtn.title = 'Xóa câu thoại này';
        removeBtn.innerHTML = TRASH_SVG;
        removeBtn.addEventListener('click', (e) => {
            const wrapperToRemove = e.currentTarget.closest(`.${ITEM_WRAPPER_CLASS}`);
            if (wrapperToRemove && !isAutomating) {
        ...
        controlsDiv.appendChild(repeatLabel);
        controlsDiv.appendChild(repeatInput);
        ...
        if (insertAfterElement && container.contains(insertAfterElement)) {
            const nextSibling = insertAfterElement.nextElementSibling;
            if (nextSibling && nextSibling !== emptyState) {
                container.insertBefore(inputWrapper, nextSibling);
            } else if (emptyState) {
                 container.insertBefore(inputWrapper, emptyState);
            } else {
    ...
    function handleMoveUp(e) {
        const currentWrapper = e.currentTarget.closest(`.${ITEM_WRAPPER_CLASS}`);
        if (!currentWrapper || isAutomating) return;
        const previousWrapper = currentWrapper.previousElementSibling;
        if (previousWrapper && previousWrapper.classList.contains(ITEM_WRAPPER_CLASS)) {
            animateSwap(currentWrapper, previousWrapper);
        }
    }
    ...
    function handleMoveDown(e) {
        ...
        if (nextWrapper && nextWrapper.classList.contains(ITEM_WRAPPER_CLASS)) {
             animateSwap(nextWrapper, currentWrapper);
        }
    }
    ...
    function animateSwap(element1, element2) {
        ...
        const margin = parseFloat(window.getComputedStyle(element1).marginBottom);

        const distance1 = rect2.top - rect1.top;
        const distance2 = rect1.bottom + margin - (rect2.bottom + margin);

        element1.style.transition = 'none';
        element2.style.transition = 'none';
        element1.style.transform = `translateY(${distance1}px)`;
        element2.style.transform = `translateY(${distance2}px)`;
        ...
        requestAnimationFrame(() => {
            element1.style.transition = `transform ${ANIMATION_DURATION_MS}ms ease-in-out`;
            element2.style.transition = `transform ${ANIMATION_DURATION_MS}ms ease-in-out`;
    ...
    function updateMoveButtonStates() {
        ...
        wrappers.forEach((wrapper, index) => {
            ...
            if (moveDownBtn) moveDownBtn.disabled = (index === wrappers.length - 1);
        });
    }

    function disableAllMoveButtons(disabled) {
        const container = document.getElementById(CONTAINER_ID);
        if (!container) return;
    ...
    function saveDialogues() {
        try {
            updateDialogueDataArray();
            GM_setValue(DIALOGUES_STORAGE_KEY, JSON.stringify(dialogueData));
        } catch (e) {
    ...
    function loadDialogues() {
        const saved = GM_getValue(DIALOGUES_STORAGE_KEY);
        const container = document.getElementById(CONTAINER_ID);
        if (!container) return;
        ...
        if (saved) {
            try {
                ...
                if (Array.isArray(parsed)) {
                    loadedData = parsed.filter(item =>
                        item && typeof item.id === 'string' && typeof item.text === 'string' && typeof item.repeat === 'number'
                    );
                    if(loadedData.length !== parsed.length) console.warn("Removed invalid saved items during load.");
                } else {
                     console.warn("Saved dialogue data format is invalid (not an array).");
                }
            } catch (e) {
                console.error("Error parsing saved dialogues:", e);
            }
        }
        ...
        if (!promptsAdded && container.querySelectorAll(`:scope > .${ITEM_WRAPPER_CLASS}`).length === 0) {
            addDialogueInput();
        }
    ...
    function saveWaitTimes() {
        ...
        minInput.value = minSeconds.toFixed(1);
        maxInput.value = maxSeconds.toFixed(1);

        GM_setValue(WAIT_MIN_SECONDS_KEY, minSeconds);
        GM_setValue(WAIT_MAX_SECONDS_KEY, maxSeconds);
    }
    ...
    function handlePanelDragStart(e) {
        if (e.target.closest('button, .header-version, input, textarea, label, .section-header, .icon-button')) {
            return;
        }
        ...
        currentPanelX = initialPanelX;
        currentPanelY = initialPanelY;
    }
    ...
    async function startAutomation() {
        updateDialogueDataArray();
        const validPromptsData = dialogueData.filter(data => data?.text?.trim());

        if (isAutomating) {
            updateStatus("Đang chạy...");
            return;
        }
        if (validPromptsData.length === 0) {
            updateStatus("Không có câu thoại hợp lệ để gửi.");
            return;
        }

        document.querySelectorAll(`.${ITEM_WRAPPER_CLASS} .${PROMPT_NUMBER_CLASS}`).forEach(span => {
        ...
        setActivePromptHighlight(-1);
        processNextPrompt();
    }

    function stopAutomation(completedSuccessfully = false) {
        isAutomating = false;
        ...
        if (completedSuccessfully) {
            ...
            updateStatus(`Hoàn thành tất cả câu thoại lúc ${timeString}`);
    ...
    function setActivePromptHighlight(index) {
        const wrappers = document.querySelectorAll(`#${CONTAINER_ID} .${ITEM_WRAPPER_CLASS}`);
        wrappers.forEach((wrapper, i) => {
            const dot = wrapper.querySelector(`.${ACTIVE_PROMPT_DOT_CLASS}`);
            const numberSpan = wrapper.querySelector(`.${PROMPT_NUMBER_CLASS}`);

            if (i === index && isAutomating) {
                 if (dot) dot.style.display = 'block';
                 if (numberSpan) numberSpan.style.display = 'none';
            } else {
                 if (dot) dot.style.display = 'none';
                 if (numberSpan) {
                     numberSpan.style.display = isAutomating ? 'none' : 'block';
                 }
            }
            wrapper.classList.remove('active-prompt');
        });
    }
    ...
    async function processNextPrompt() {
        ...
        setActivePromptHighlight(currentPromptIndex);
        const currentData = dialogueData[currentPromptIndex];

        if (!currentData || typeof currentData.repeat !== 'number' || !currentData.text) {
            console.error("Dữ liệu prompt không hợp lệ tại chỉ mục:", currentPromptIndex, currentData);
        ...
        if (currentRepeatCount >= totalRepeatsNeeded) {
            ...
            currentRepeatCount = 0;
            setActivePromptHighlight(-1);
            setTimeout(processNextPrompt, 50);
            return;
        }
        ...
        try {
            ...
            if (stopRequested) {
                 console.log("Stop requested after sending prompt.");
                 return;
             }
            ...
            if (stopRequested) {
                 console.log("Stop requested while waiting for response.");
                 return;
            }
            ...
            const maxWaitMs = Math.max(minWaitMs, maxWaitSec * 1000);
            ...
            if (stopRequested) {
                 console.log("Stop requested during wait delay.");
                 return;
             }
    ...
    async function sendPromptToChatGPT(text) {
        ...
        if (currentContent) {
            textarea.value = '';
            const pTagClear = textarea.querySelector('p');
            if (pTagClear) pTagClear.textContent = '';
        ...
        const editorInstance = textarea.cmView?.view;
        let success = false;
        ...
        if (editorInstance?.dispatch) {
            try {
                ...
                success = true;
                console.log("Used CodeMirror dispatch to set text.");
            } catch (e) {
                console.warn("CodeMirror dispatch failed, trying fallback:", e);
            }
        }
        ...
        if (!success && pTag) {
             try {
                 ...
                 textarea.dispatchEvent(new Event('change', { bubbles: true }));
                 success = true;
                 console.log("Used pTag.textContent to set text.");
             } catch (e) {
                 console.warn("Setting pTag textContent failed, trying final fallback:", e);
             }
         }
        ...
        if (!success) {
            try {
                ...
                textarea.dispatchEvent(new Event('change', { bubbles: true }));
                success = true;
                console.log("Used textarea.value to set text.");
            } catch (e) {
        ...
        for (let i = 0; i < findAttempts; i++) {
            ...
            if (i < findAttempts - 1) {
                 console.log(`Send button not ready, attempt ${i+1}/${findAttempts}. Retrying...`);
    ...
    async function waitForChatGPTResponse() {
        return new Promise((resolve, reject) => {
            ...
            const noStopButtonThreshold = 16;
            ...
            const intervalId = setInterval(() => {
                ...
                const isGenerating = !!stopButton && stopButton.offsetHeight > 0;

                if (isGenerating) {
                    if (!generationDetected) {
                        console.log("ChatGPT response generation detected.");
                        generationDetected = true;
                    }
                    noStopButtonCount = 0;
                } else {
                    if (generationDetected) {
                        console.log("Stop generating button disappeared. Assuming response complete.");
                        clearInterval(intervalId);
                        resolve();
                    } else {
                        noStopButtonCount++;
                        if (noStopButtonCount >= noStopButtonThreshold) {
                            console.warn(`Không phát hiện nút '${STOP_GENERATING_BUTTON_SELECTOR}' sau khi gửi. Giả định hoàn thành/lỗi.`);
                            clearInterval(intervalId);
                            resolve();
                        }
                    }
    ...
    function downloadCodeblockData() {
        ...
        if (!targetId) {
            updateStatus("Vui lòng nhập ID Code Block.");
            return;
        }
        ...
        codeBlockContainers.forEach(container => {
            ...
            if (header && header.textContent.trim() === targetId && !container.closest(`#${MAIN_PANEL_ID}`)) {
    ...
    GM_addStyle(`
        ...
        @keyframes pulsate-red {
         ...
         #${MAIN_PANEL_ID} {
            ...
            visibility: visible;
            transform: scale(1);
            transition: transform var(--animation-duration) ease-in-out, opacity var(--animation-duration) ease-in-out, visibility 0s linear 0s;
            will-change: transform, opacity, visibility;
        }
         #${MAIN_PANEL_ID}.minimizing {
            transform: scale(0.05) translate(-15px, -15px);
            opacity: 0;
        }
         #${MAIN_PANEL_ID}.minimized {
            ...
            pointer-events: none;
            visibility: hidden;
            transition: transform var(--animation-duration) ease-in-out, opacity var(--animation-duration) ease-in-out, visibility 0s linear var(--animation-duration);
        }
         ...
         #${MINIMIZE_DOT_ID} {
            ...
            bottom: 15px;
            right: 15px;
            width: var(--minimize-dot-size);
            height: var(--minimize-dot-size);
            background-color: var(--minimize-dot-bg);
            border-radius: 50%;
            box-shadow: var(--minimize-dot-shadow);
            cursor: pointer;
            z-index: 10002;
            display: flex;
            align-items: center;
        ...
        .section-header {
            ...
            padding: 11px 15px; font-size: 1.05em; font-weight: 600;
        ...
        #dialogue-empty-state {
            ...
            padding: 25px 15px; text-align: center; color: var(--text-secondary);
            flex-grow: 1;
            user-select: none; order: 999;
        }
        ...
        .add-prompt-main-btn {
            width: 100%; padding: 8px 11px; font-size: 1em;
            border-radius: var(--border-radius-inner);
            background-color: transparent; color: var(--text-secondary);
            border: 1px dashed rgba(255, 255, 255, 0.25);
            cursor: pointer; transition: all 0.2s ease;
            display: inline-flex; align-items: center; justify-content: center; gap: 5px; font-weight: 500;
        }
        .add-prompt-main-btn:hover:not(:disabled) {
            background-color: rgba(255, 255, 255, 0.06); color: var(--text-primary);
        ...
        .${ITEM_WRAPPER_CLASS} {
            ...
            position: relative;
            overflow: visible;
            transition: transform var(--animation-duration) ease-in-out, background-color 0.2s ease, border-color 0.2s ease;
            padding-left: 26px;
        }
        ...
        .${ACTIVE_PROMPT_DOT_CLASS} {
            ...
            border-radius: 50%;
            box-shadow: 0 0 5px 1px rgba(255, 77, 79, 0.7);
            display: none;
            animation: pulsate-red 1.8s infinite ease-in-out;
            z-index: 2;
        }
        ...
        .dialogue-textarea:focus {
            ...
            background-color: var(--input-focus-bg); box-shadow: var(--input-focus-shadow);
        }
        ...
        .repeat-input {
            width: 38px; padding: 3px 5px; margin-right: auto;
        ...
        .icon-button {
            background: transparent; border: none; color: var(--icon-color); padding: 4px;
        ...
        .icon-button:disabled { opacity: 0.3; cursor: not-allowed; background-color: transparent !important; color: var(--icon-color) !important; }
        ...
        #automation-settings {
            padding: 10px 15px 8px 15px; display: flex; align-items: center; gap: 6px;
            flex-shrink: 0; font-size: 1em; color: var(--text-secondary);
        }
        #automation-settings label.wait-label { margin: 0; flex-shrink: 0; line-height: 1; }
        #automation-settings input[type="number"] {
            width: 42px; padding: 5px 6px; border: 1px solid var(--input-border);
            border-radius: 6px; background-color: var(--input-bg); color: var(--text-primary);
            font-size: 1em; text-align: center; -moz-appearance: textfield; transition: all 0.2s ease;
        }
        ...
        #automation-settings input[type="number"]:focus {
            outline: none; border-color: var(--input-focus-border);
            background-color: var(--input-focus-bg); box-shadow: var(--input-focus-shadow);
        }
        ...
        #start-automation-btn, #stop-automation-btn {
            padding: 8px 12px; font-size: 1.05em; flex-grow: 1; margin: 0;
            border-radius: var(--border-radius-inner); font-weight: 500;
        ...
        #stop-automation-btn {
            background-color: var(--stop-button-bg); color: var(--stop-button-text);
        ...
        #automation-status {
            padding: 9px 15px 10px 15px; font-size: 0.95em; text-align: center;
            color: var(--text-secondary); width: 100%; line-height: 1.4;
            min-height: calc(1.4em + 3px);
            flex-shrink: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
            box-sizing: border-box; border-top: 1px solid var(--divider-color);
            background-color: rgba(0,0,0,0.04);
        }

        #download-section {
            padding: 12px 15px 14px 15px; display: flex; align-items: center; gap: 9px;
            flex-shrink: 0; font-size: 1em; background-color: transparent;
        }
        ...
        #download-section input[type="text"] {
            flex-grow: 1; padding: 6px 10px; border: 1px solid var(--input-border);
            border-radius: 6px; background-color: var(--input-bg); color: var(--text-primary);
            font-size: 1em; min-width: 60px; transition: all 0.2s ease;
        }
        ...
        #download-section button {
            padding: 6px 10px; font-size: 1em; border-radius: 6px;
            background-color: rgba(80, 140, 255, 0.18);
            color: #cddbff; border: 1px solid rgba(80, 140, 255, 0.4);
            cursor: pointer; transition: all 0.2s ease;
        ...
        #download-section button:hover:not(:disabled) {
            ...
            border-color: rgba(80, 140, 255, 0.6); color: #e0ecff;
        }
        #download-section button:active:not(:disabled) { transform: scale(0.97); }
        #download-section input:disabled, #download-section button:disabled {
            cursor: not-allowed; opacity: 0.6; background-color: rgba(255,255,255,0.05);
            box-shadow: none; border-color: var(--input-border); color: var(--text-placeholder);
        }
        #download-section button:disabled {
            background-color: rgba(80, 140, 255, 0.1); border-color: rgba(80, 140, 255, 0.2); color: rgba(205, 219, 255, 0.6);
        }
