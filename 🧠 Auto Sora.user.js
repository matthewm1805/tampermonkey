...
// @version      5.0
// @description  Auto generate prompt list, bulk download (PNG, multithreaded), single file download, auto crop (16:9, 9:16, 1:1), H/V/Square filter, Stop button, Find Similar Image, Auto-Submit (with UI disable toggle), Realtime Prompt Count, Glass UI, No prompt scrollbar, Enhanced Logging
// @author       Matthew M.
// @match        *://sora.com/*
// @match        *://www.sora.com/*
// @match        *://www.sora.*.com/*
...
(function () {
    ...
    function log(msg) {
        ...
        console.log(`[Auto Sora v5.0 ${timestamp}] ${msg}`);
    }
    ...
    function toggleCooldownInputState() {
        ...
        const cooldownLabel = cooldownInput?.previousElementSibling;

        if (!autoCheckbox || !cooldownInput) return;
        const isAuto = autoCheckbox.checked;
        // log(`Toggling Cooldown input state. Auto checkbox checked: ${isAuto}`); // Reduced logging noise
        ...
        if (isAuto) {
            ...
            // log("Cooldown input disabled."); // Reduced logging noise
        } else {
            ...
            // log("Cooldown input enabled."); // Reduced logging noise
        }
    }

    // NEW Function to update the prompt count on the start button
    function updateStartButtonPromptCount() {
        ...
        if (!textarea || !startButton) return;

        const prompts = textarea.value.split('\n').map(x => x.trim()).filter(Boolean);
        const count = prompts.length;
        startButton.textContent = `▶ Bắt đầu (${count})`;
        // log(`Start button prompt count updated: ${count}`); // Can be noisy, uncomment if needed
    }
    ...
    function createUI() {
        ...
        // Add listener for the prompt input to update button count
        ...
        // Set initial state for Cooldown input and Start button count
        toggleCooldownInputState();
        updateStartButtonPromptCount(); // Set initial count
        createAuxiliaryUI();
        log("Auxiliary UI created.");
    }
    ...
    // --- Functions from 4.11 ---
         function handleClear() { log("Clear button clicked."); document.getElementById('sora-input').value = ''; updateStartButtonPromptCount(); log("Prompt input cleared and button count updated."); } // Added count update on clear
    ...
    function handleStop() {
        ...
        if (autoSubmitTimeoutId) {
            ...
            log("Cleared pending auto-submit timeout.");
        }
        ...
        if (countdownInterval) {
            ...
            if (cooldownBtn) cooldownBtn.textContent = 'Cooldown: --';
            log("Cooldown timer cleared.");
        }
    ...
    function updateProgress() {
        ...
        if (isRunning) {
            ...
            // REMOVED: if (isGenerating) { statusText += " (Đang tạo...)"; }
            ...
            setTimeout(() => {
                if (!isRunning) {
                    ...
                    if (wrapper && wrapper.style.display === 'none' && miniBtn) {
                        ...
                        log("Auxiliary UI hidden, mini button shown after completion/stop.");
                    }
    ...
    // Only used if Auto-Submit is OFF
    function startCountdown() {
        let timeRemaining = cooldownTime;
        const cooldownBtn = document.getElementById('sora-cooldown');
        if (!cooldownBtn || !isRunning || isGenerating) {
            log(`startCountdown skipped (running: ${isRunning}, generating: ${isGenerating}, btn: ${!!cooldownBtn})`);
            return;
        }

        log(`Starting manual cooldown timer: ${timeRemaining}s`);
        cooldownBtn.textContent = `⏳ ${timeRemaining}s`;
        cooldownBtn.style.display = 'inline-block';
        ...
        countdownInterval = setInterval(() => {
            if (!isRunning || isGenerating) {
                clearInterval(countdownInterval);
                countdownInterval = null;
                const currentBtn = document.getElementById('sora-cooldown');
                if (currentBtn) {
                    currentBtn.textContent = `⏳ --`;
                    currentBtn.style.display = 'none';
                }
                log(`Countdown stopped/interrupted (running: ${isRunning}, generating: ${isGenerating}).`);
                return;
            }
            timeRemaining--;
            const currentBtn = document.getElementById('sora-cooldown');
            if (currentBtn) currentBtn.textContent = `⏳ ${timeRemaining}s`;

            if (timeRemaining <= 0) {
                clearInterval(countdownInterval);
                countdownInterval = null;
                log("Manual cooldown finished.");
                if (currentBtn) {
                    currentBtn.textContent = `⏳ 0s`;
                    setTimeout(() => { if (currentBtn && !isGenerating) currentBtn.style.display = 'none'; }, 500);
                }
                processNextPrompt(); // Process next prompt when timer finishes
            }
        }, 1000);
    }
    ...
    function submitPrompt(prompt) {
        ...
        log(`Submitting prompt ${totalPromptCount - promptQueue.length}/${totalPromptCount}: ${prompt.substring(0, 50)}...`);
        ...
        if (key && textarea[key]?.onChange) {
            try { log("Triggering React onChange..."); textarea[key].onChange({ target: textarea }); }
            catch (e) { log("ERROR triggering React onChange:"); console.error(e); }
        } else { log("WARNING: React onChange handler not found for textarea."); }

        log("Waiting 600ms for submit button to enable...");
        setTimeout(() => {
            if (!isRunning) { log("Submit button click cancelled: Not running."); return; }
            const btn = document.querySelector('button[data-disabled="false"][class*="bg-token-bg-inverse"]');
            if (btn) {
                log("Submit button found and enabled. Setting flags, starting observer, clicking...");
                isGenerating = true;
                _generationIndicatorRemoved = false;
                _newImagesAppeared = false;
                updateProgress();
                ...
                if (btnKey && btn[btnKey]?.onClick) {
                    try { btn[btnKey].onClick({ bubbles: true, cancelable: true, isTrusted: true }); log("React onClick triggered."); }
                    catch (e) { log("ERROR triggering React onClick:"); console.error(e); log("Attempting standard .click() as fallback..."); btn.click(); }
     ...
     function handleGenerationComplete() {
        if (!isGenerating || !isRunning) {
            log(`handleGenerationComplete called but state is not correct (generating: ${isGenerating}, running: ${isRunning}). Ignoring.`);
            return;
        }

        log("Generation complete confirmed by observer. Handling next step...");
        isGenerating = false;
        completionObserver?.disconnect();
        log("Completion observer disconnected.");
        updateProgress();
        ...
        if (isAuto) {
            if (promptQueue.length > 0) {
             ...
             log("Manual mode: Generation finished.");
             if (promptQueue.length > 0) {
                 log("Manual mode: Starting cooldown timer for next prompt.");
                 startCountdown();
             } else {
                 log("Manual mode: Queue empty after generation. Process finished.");
                 isRunning = false;
                 updateProgress();
             }
        }
    ...
    function processNextPrompt() {
        ...
        if (isGenerating) { log("processNextPrompt: Aborted, already generating."); return; }
        if (promptQueue.length === 0) { log("processNextPrompt: Queue is empty. Finishing run."); isRunning = false; updateProgress(); return; }

        const nextPrompt = promptQueue.shift();
        log(`Processing prompt ${totalPromptCount - promptQueue.length + 1}/${totalPromptCount}...`);
        submitPrompt(nextPrompt);
        updateProgress();
    }

    function startLoop() {
        if (!isRunning || promptQueue.length === 0) { log("startLoop: Condition not met."); isRunning = false; updateProgress(); return; }
        log(`Starting loop: Submitting first prompt (1/${totalPromptCount}).`);
        processNextPrompt();
    }


    // --- Download Logic ---
    function updateSelectedCount() { const count = selectedImageUrls.size; /* log(`Updating download count: ${count}`); */ try { const btnText = document.getElementById("sora-download-text"); const btn = document.getElementById("sora-download-images"); const icon = document.getElementById("sora-download-icon"); const errorEl = document.getElementById("sora-download-error"); if (btnText && btn && !isDownloading) { btnText.textContent = `Tải hình (${count})`; btn.disabled = (count === 0); if (icon) icon.style.display = 'inline'; if (errorEl) errorEl.textContent = ''; } else if (btn) { btn.disabled = true; } } catch (e) { log("ERROR updating selected count UI:"); console.error(e); } const btn = document.getElementById("sora-download-images"); if (btn && !isDownloading) { btn.disabled = (selectedImageUrls.size === 0); } }
    ...
    // --- Checkbox Insertion and Image Observation (Revised) ---
    ...
    function insertCheckbox(img) {
        try {
            ...
            const gridItem = img.closest('div[data-index]'); if (!gridItem) return;
            if (gridItem.querySelector('.sora-image-checkbox')) return; // Prevent duplicates

            //log(`Inserting checkbox for image in grid item ${gridItem.getAttribute('data-index')}`);

            const checkbox = document.createElement("input");
            checkbox.type = "checkbox"; checkbox.className = "sora-image-checkbox";
            Object.assign(checkbox.style, { position: "absolute", top: "8px", left: "8px", zIndex: "10", width: "18px", height: "18px", cursor: "pointer", transform: "scale(1.3)", accentColor: "#4a90e2" });
            ...
            checkbox.addEventListener("change", (e) => {
                ...
                // log(`${action} image via checkbox: ${img.src.substring(img.src.length - 20)}`);
                updateSelectedCount();
            });

            if (window.getComputedStyle(gridItem).position === 'static') { gridItem.style.position = 'relative'; }
            gridItem.insertBefore(checkbox, gridItem.firstChild);

            if (img.complete && img.naturalWidth > 0) { setInitialCheckboxState(); }
            else { img.removeEventListener('load', setInitialCheckboxState); img.removeEventListener('error', handleImageError); img.addEventListener('load', setInitialCheckboxState, { once: true }); img.addEventListener('error', handleImageError, { once: true }); }
        } catch (e) { log(`ERROR inserting checkbox for image: ${img?.src?.substring(0, 50)}...`); console.error(e); }
    }
    imageObserver = new MutationObserver((mutations) => {
        let imagesToCheck = new Set();
        // log(`ImageObserver triggered with ${mutations.length} mutations.`); // Can be very noisy
                ...
                for (const node of mutation.addedNodes) {
                    if (node.nodeType === 1) {
                        if (node.matches && node.matches('div[data-index]')) {
                            ...
                            if (img && !node.querySelector('.sora-image-checkbox')) { imagesToCheck.add(img); }
                        } else if (node.querySelectorAll) {
                            node.querySelectorAll('div[data-index]').forEach(gridItem => {
                                const img = gridItem.querySelector('a > img');
                                if (img && !gridItem.querySelector('.sora-image-checkbox')) { imagesToCheck.add(img); }
                            });
                        }
                    }
                }
    ...
    // Observer for detecting generation completion
    completionObserver = new MutationObserver((mutations) => {
        if (!isGenerating) return;
        //log(`CompletionObserver triggered (isGenerating=${isGenerating}). Flags: removed=${_generationIndicatorRemoved}, appeared=${_newImagesAppeared}`); // Can be noisy
            ...
            if (mutation.type === 'childList') {
                mutation.removedNodes.forEach(node => {
                    if (node.nodeType === 1 && node.querySelector && node.querySelector('svg[class*="desktop:h-20"] circle[class*="-rotate-90"]')) {
                        log("CompletionObserver: Detected REMOVAL of progress indicator structure.");
                        foundIndicatorRemoval = true;
                         ...
                         log("CompletionObserver: Detected REMOVAL of progress text node.");
                         foundIndicatorRemoval = true; // Consider text removal as indicator removal too
                    }
                });
                ...
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1) {
                         if ((node.matches && node.matches('div[data-index="0"]')) || (node.querySelector && node.querySelector('div[data-index="0"]'))) {
                             log(`CompletionObserver: Detected ADDITION of new image container [data-index=0].`);
                             foundNewImage = true;
                         }
                    }
                 });
        ...
        if (isGenerating && _generationIndicatorRemoved && _newImagesAppeared) {
    ...
    function handleDocumentClickForSimilar(event) {
        ...
        const soraGenRegex = /^https?:\/\/sora\.com\/g\/(gen_[a-zA-Z0-9]+)/; // Matches sora.com/g/gen_...
        const match = link.href.match(soraGenRegex);
        if (match && match[1]) {
            const genId = match[1];
            const exploreUrl = `${window.location.origin}/explore?query=${genId}`; // Use origin for flexibility
            log(`Find Similar Mode: Match found (${genId}). Opening with window.open: ${exploreUrl}`);
            event.preventDefault();
            event.stopPropagation();
            window.open(exploreUrl, '_blank'); // Use standard window.open as @grant is none
        }
    }
    ...
    waitForElement('textarea[placeholder*="Describe"], textarea.flex.w-full', () => {
        try {
            ...
            createUI(); // This now includes setting initial button count and adding listeners
            log("UI creation function finished.");
